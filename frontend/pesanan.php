        ...
        <?php if (count($orders) > 0): ?>
            <div class="order-table">
                <table class="table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="<?= ($order_success && $order_success['id_order'] == $order['id_order']) ? 'table-success' : '' ?>">
                            <td>
                            ...
                            <td>
                                <div class="d-inline-flex" style="gap: 6px;">
                                    <button class="btn btn-primary-cipamilk btn-sm" data-toggle="modal" data-target="#orderModal<?= $order['id_order'] ?>" title="Lihat Detail">
            ...
            <div class="modal fade" id="orderModal<?= $order['id_order'] ?>" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content" style="border-radius: var(--radius-md); border: none; overflow: hidden; box-shadow: 0 15px 40px rgba(0,0,0,0.15);">
                        <div class="modal-header" style="background: linear-gradient(135deg, var(--primary), var(--primary-dark)); color: #fff;">
                            <h5 class="modal-title font-weight-bold" style="font-family: var(--font-heading); font-size: 1.1rem;">
                        ...
                        <div class="modal-body" style="padding: 1.5rem;">
                            ...
                            <div class="p-3 mb-3" style="background: #f8fafc; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 0.88rem;">
                                <div class="text-muted mb-1"><i class="fas fa-map-marker-alt mr-1" style="color: var(--primary);"></i> <strong>Alamat Pengiriman:</strong></div>
                                <div><?= nl2br(htmlspecialchars($order['alamat_pengiriman'])) ?></div>
                                <?php if (!empty($order['nomor_telepon'])): ?>
                                    <div class="mt-1 text-muted"><i class="fas fa-phone mr-1"></i> <?= htmlspecialchars($order['nomor_telepon']) ?></div>
                                <?php endif; ?>
                            </div>
                            <?php endif; ?>
                                ...
                                <div class="d-flex align-items-center">
                                    <?php if (!empty($d['gambar'])): ?>
                                        <img src="<?= $base_url ?>/assets/images/products/<?= htmlspecialchars($d['gambar']) ?>" 
                                             alt="" style="width: 42px; height: 42px; object-fit: cover; border-radius: 8px; margin-right: 12px; border: 1px solid #e2e8f0;">
                                    <?php endif; ?>
                                    <div>
                                        <strong style="font-family: var(--font-heading); font-size: 0.95rem;"><?= htmlspecialchars($d['nama_produk']) ?></strong>
                                        <br><small class="text-muted"><?= formatRupiah($d['subtotal'] / $d['jumlah']) ?> &times; <?= $d['jumlah'] ?></small>
                                    </div>
                                </div>
                            ...
                            <div class="d-flex justify-content-between mt-3 pt-2" style="border-top: 2px dashed #e2e8f0;">
                                ...
                                <strong style="font-size: 1.15rem; color: var(--primary);"><?= formatRupiah($order['total']) ?></strong>
                            </div>
                        </div>
...
<div class="modal fade modal-order-success" id="orderSuccessModal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <h3 class="font-weight-bold mb-2" style="color: #2c3e50; font-family: var(--font-heading); font-size: 1.5rem;">
                    Pesanan Berhasil Dibuat!
                </h3>
                <p class="text-muted mb-4" style="font-size: 0.95rem; line-height: 1.5;">
                    Terima kasih telah berbelanja di <strong>Cipamilk</strong>. Pesanan Anda telah tercatat dan segera kami siapkan.
                </p>
                ...
                <div class="order-success-summary">
                    ...
                    <div class="summary-row">
                        ...
                        <span class="text-truncate text-right" style="max-width: 220px; font-size: 0.88rem; font-weight: 600;" title="<?= htmlspecialchars($order_success['alamat']) ?>">
                    ...
                    <div class="summary-row">
                        ...
                        <span class="badge badge-warning text-uppercase" style="border-radius: 20px; padding: 0.35rem 0.75rem; font-weight: 700; font-size: 0.75rem;">
                            Menunggu Konfirmasi
                        </span>
                    </div>
                </div>
